...
def main(cfg, local_rank):
    ...
    for epoch in range(start_epoch + 1, cfg.TRAIN.EPOCHS + 1):
        ...
        if cfg.TRAIN.VAL_INTERVALS > 0 and epoch % cfg.TRAIN.VAL_INTERVALS == 0:
            save_model(os.path.join(cfg.OUTPUT_DIR, cfg.EXP_ID, 'model_{}.pth'.format(mark)), 
                 epoch, model, optimizer)
            with torch.no_grad():
                log_dict_val = trainer.val(epoch, val_loader, val_dataset)
                print('mAP is: ', mAP)
            
            # save best performing model
            if mAP > best:
                best = mAP
                save_model(os.path.join(cfg.OUTPUT_DIR, cfg.EXP_ID, 'model_best.pth'), 
                           epoch, model)
        else:
            save_model(os.path.join(cfg.OUTPUT_DIR, cfg.EXP_ID, 'model_epoch_{}.pth'.format(epoch)), 
                     epoch, model)
            save_model(os.path.join(cfg.OUTPUT_DIR, cfg.EXP_ID, 'model_last.pth'), 
                     epoch, model)

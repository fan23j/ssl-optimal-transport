...
class SimCLRPreTrainer(BaseTrainer):
    ...
    def train(self, epoch, data_loader):
        ...
        for ((pos_1, pos_2), _) in train_bar:
            pos_1, pos_2 = pos_1.to(device=torch.device('cuda:%d'%self.local_rank), non_blocking=True), pos_2.to(device=torch.device('cuda:%d'%self.local_rank), non_blocking=True)
            ...
            train_bar.set_description('Train Epoch: [{}/{}] {}'.format(epoch, self.cfg.TRAIN.EPOCHS, loss_state_str))
        ...
        with torch.no_grad():
            ...
            for data, _, target in test_bar:
                ...
                test_bar.set_description('Test Epoch: [{}/{}] Acc@1:{:.2f}% Acc@5:{:.2f}%'
                                        .format(epoch, self.cfg.TRAIN.EPOCHS, total_top1 / total_num * 100, total_top5 / total_num * 100))
